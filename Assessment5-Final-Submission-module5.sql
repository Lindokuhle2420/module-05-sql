...
-- Scenario 18
select customers.customer_name,
Count(orders.status) as number_of_completed_orders,
orders.quantity as total_quantity_purchased,
products.price*orders.quantity as Total_amount_spent
from customers
inner join orders on customers.customer_id = orders.customer_id
inner join products on orders.product_id = products.product_id
where orders.status = 'Completed'
GROUP BY customers.customer_name,orders.status,orders.quantity,products.price;

-- Scenario 19
select products.product_name,orders.quantity as total_quantity_ordered
from orders
inner join products on orders.product_id = products.product_id
where orders.quantity > 4  
GROUP by products.product_name,orders.order_id;

-- Scenario 20
select customers.customer_name,Count(order_id)from customers
...
-- Scenario 21
select customers.city,Count(orders.status) as number_of_completed_orders,
orders.quantity as Total_units_purchased,
products.price*orders.quantity as Total_sales_value
from customers
inner join orders on customers.customer_id = orders.customer_id
inner join products on orders.product_id = products.product_id
where orders.status = 'Completed'
GROUP BY customers.city,products.price,customers.customer_id,orders.quantity;


-- Scenario 22
select products.product_name,products.category,Count(orders.status) as number_of_orders,
orders.quantity as Total_units_sold,products.price*orders.quantity as Total_revenue
from customers
inner join orders on customers.customer_id = orders.customer_id
inner join products on orders.product_id = products.product_id
where orders.status = 'Completed' and orders.quantity > 2
GROUP BY products.product_name,orders.order_id
